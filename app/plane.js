    ...
    const __MODULE__ = Erlang.atom('Plane');
    ...
    let init = fun([[], function()    {
        ...
        socket0.onOpen(fun([[fun.parameter], function(ev)    {
        return     console.debug('OPEN',ev);
      }]));
        socket0.onError(fun([[fun.parameter], function(ev)    {
        return     console.debug('ERROR',ev);
      }]));
        socket0.onClose(fun([[fun.parameter], function(ev)    {
        return     console.debug('CLOSE',ev);
      }]));
        let [chan0] = fun.bind(fun.parameter,socket0.channel('rooms:lobby',{}));
        JS.get_property_or_call_function(chan0,'join').receive('ignore',fun([[], function()    {
        return     console.error('auth error');
      }])).receive('ok',fun([[], function()    {
        ...
        chan0.on('new:msg',fun([[fun.parameter], function(msg)    {
        ...
        return     chan0.on('user:entered',fun([[fun.parameter], function(msg)    {
